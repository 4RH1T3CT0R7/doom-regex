...
def build_rules() -> list[tuple[str, str, str]]:
    ...
    # fetch — ЦИФРОВОЕ ДЕРЕВО: #P плотный (слот РОВНО 23 симв), позиция
    # слота pc = 23*pc; прыжок = сумма фикс-прыжков по цифрам PC.
    # Ветвление по значению цифры — БИНАРНОЕ: при чтении цифры атомарно
    # ставятся 4 пустые маркер-группы (?<qib>) по битам (класс цифр с
    # установленным битом b), прыжок включается условием (?(qib)...)
    # «маркер стоял». Сумма прыжковых длин минимальна: 23*(2^20-1) ~ 24М
    # символов (компилятор python-regex платит ~150нс/символ прыжка —
    # 15-условная версия на 193М не компилировалась). Табличных
    # lookup'ов нет (зона #D лежит ДО #P — из середины #P она
    # lookahead'у недоступна). Атомик (?>...) вокруг маркеров обязателен:
    # без него NOSLOT-несматч перебирал бы 2^20 бит-комбинаций.
    BITCLS = {3: "[89a-f]", 2: "[4-7c-f]", 1: "[2367abef]", 0: "[13579bdf]"}

    def jump(n: int) -> str:
        # фикс-прыжок без повторов групп: большие {n}-повторы взрывают
        # компилятор python-regex (MemoryError) — только конкатенация
        # атомов .{65535} + остаток. (?s:) обязателен: не-dotall '.'
        # заставляет PCRE2 JIT сканировать каждый символ на \n (6мс/хит
        # на 90МБ), dotall — это указатель += n за O(1)
        if n == 0:
            return ""
        k, r2 = divmod(n, 65535)
        return "(?s:" + ".{65535}" * k + (rf".{{{r2}}}" if r2 else "") + ")"

    def pc_digit_cap(i: int) -> str:
        if i >= 5:
            # программа < 16^5 слотов: у валидного PC старшие цифры 0;
            # иначе прыжка нет -> адрес слота не совпадёт -> trap_noslot
            return rf"(?<p{i}>.)"
        marks = "".join(
            rf"(?>(?={BITCLS[b]})(?<q{i}{b}>)|)" for b in range(3, -1, -1))
        return marks + rf"(?<p{i}>.)"

    def pc_digit_jumps(i: int) -> str:
        return "".join(
    ...
    R.append(("fetch",
              ...
              + r":(?<nci>.{12});)",
              "RVM1|ST:run|PH:0|CI:${nci}"
              "|PC:${p7}${p6}${p5}${p4}${p3}${p2}${p1}${p0}"))
    ...
    # --- I/O (v2.0 Э2: IN/OUT в хвосте строки, за #E) -----------------------
    # putc/getc НЕ горячие (в хвосте кадра IO нет); ${pre}/${mid} —
    # гигантские группы через всю строку, но identity-skip драйвера их
    # не копирует (src==dst), а pure-режим остаётся эталоном.
    R.append(("putc",
              ci(0x40, r"(?<d>[0-7]).{9}")
              + read_reg(r"(?P=d)", r".{6}(?<b1>.)(?<b0>.)")
              + rf"(?<pre>(?>{HOP}E)\|IN:[0-9a-f]*+\|OUT:[0-9a-f]*+)\|Z",
              R1 + "${pre}${b1}${b0}|Z"))
    R.append(("getc_in",
              ci(0x41, r"(?<d>[0-7]).{9}")
              + rf"(?<pre>{FIELD}R(?P=d):).{{8}}"
              + rf"(?<mid>(?>{HOP}E)\|IN:)(?<i1>[0-9a-f])(?<i0>[0-9a-f])",
              R1 + "${pre}000000${i1}${i0}${mid}"))
    R.append(("getc_eof",
              ci(0x41, r"(?<d>[0-7]).{9}")
              + rf"(?=(?>{HOP}E)\|IN:\|)" + consume_dst(),
              R1 + "${pre}00000000"))
